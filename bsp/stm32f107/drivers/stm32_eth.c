...
struct pbuf* stm32_eth_rx(rt_device_t dev)
{
    ...
    {      
        ...
        if (p != RT_NULL)
        {
            ...
            {
                memcpy((u8_t*)q->payload, from, q->len);
                from += q->len;
            }    
        }
        #ifdef ETH_RX_DUMP
...
rt_err_t stm32_eth_tx(rt_device_t dev, struct pbuf* p)
{
    struct pbuf *q;
...
rt_err_t stm32_eth_init(rt_device_t dev)
{
...
rt_err_t stm32_eth_open(rt_device_t dev, rt_uint16_t oflag)
{
    rt_set_errno(-RT_ENOSYS);
    return RT_EOK;
}

rt_err_t stm32_eth_close(rt_device_t dev)
{
    rt_set_errno(-RT_ENOSYS);
    return RT_EOK;
}
rt_size_t stm32_eth_read(rt_device_t dev, rt_off_t pos, void *buffer, rt_size_t size)
{
    rt_set_errno(-RT_ENOSYS);
    return RT_EOK;
}

rt_size_t stm32_eth_write(rt_device_t dev, rt_off_t pos, const void *buffer, rt_size_t size)
{
    rt_set_errno(-RT_ENOSYS);
    return RT_EOK;
}

rt_err_t stm32_eth_control(rt_device_t dev, rt_uint8_t cmd, void *args)
{
    switch(cmd)
    {
        case NIOCTL_GADDR:
...
void rt_hw_stm32_eth_init(void)
{
    ...
    stm32_eth_device.parent.parent.init = stm32_eth_init;
    stm32_eth_device.parent.parent.open = stm32_eth_open;
    stm32_eth_device.parent.parent.close = stm32_eth_close;
    stm32_eth_device.parent.parent.read = stm32_eth_read;
    stm32_eth_device.parent.parent.write = stm32_eth_write;
    stm32_eth_device.parent.parent.control = stm32_eth_control;
    stm32_eth_device.parent.parent.user_data = RT_NULL;

    stm32_eth_device.parent.eth_tx = stm32_eth_tx;
    stm32_eth_device.parent.eth_rx = stm32_eth_rx;
